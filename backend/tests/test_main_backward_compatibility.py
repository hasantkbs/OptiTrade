...
"""
EXPECTED_ROUTE_COUNT = 134  # 133 pre-existing routes + 1 new POST /api/v1/signals/alerts route
...
def test_openapi_generation_still_works_and_includes_legacy_and_new_paths(client):
    ...
    assert "/portfolio/optimize" in schema["paths"]
    assert "/portfolios" in schema["paths"]
    assert "/portfolios/{portfolio_id}/dashboard" in schema["paths"]
    assert "/health" in schema["paths"]
    assert "/auth/register" in schema["paths"]
    assert "/users/me" in schema["paths"]
    assert "/organizations" in schema["paths"]
    assert "/paper-trading/accounts" in schema["paths"]
    assert "/dashboard/overview" in schema["paths"]
