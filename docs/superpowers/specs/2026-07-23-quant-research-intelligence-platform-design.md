...
## Final Revision (2026-07-23) — Pre-Freeze Amendments

Four decisions, incorporated before Phase 1 is frozen. Each is cross-referenced to the
specific finding it resolves in `2026-07-23-phase1-final-review-and-verdict.md`. Design
only — no code, no implementation, no API design.

### Amendment 1 — LLMs become Explanation Engines, not Voting Engines

**Change**: Trader Profile and Investor Profile are removed from the Decision Engine's
voting roster entirely. The Engine Contract now splits into two tiers:

**Tier A — Voting Engines** (deterministic or statistical/ML only): Technical,
Fundamental, News, and future Volume/Market Structure/Macro/Cross-Asset/Relative-Strength
engines, plus any future GBM/ensemble/temporal statistical model per the model-evolution
roadmap. Each still produces the full `EngineOutput` contract (score, confidence, evidence,
explanation, historical_accuracy) and participates in the Decision Engine's
accuracy-weighted voting exactly as originally designed (§4 above).

**Tier B — Explanation Layer** (LLM-based, strictly downstream of the Decision Engine):
Trader Profile, Investor Profile. These consume (a) the Decision Engine's `DecisionOutput`
(the final BUY/SELL/HOLD, confidence, and its deterministic weight-table explanation) and
(b) the raw `evidence` from every Tier A engine that contributed — and produce a
human-readable, Turkish-language narrative report. They produce **no** score/confidence
that feeds back into any decision. Their existing internal reasoning style (technical
trader perspective, then macro investor perspective, then synthesis) is preserved, but is
now understood as *narrative synthesis over a decision already made deterministically*,
never as the LLM making its own call.

**Consequence for existing product surfaces**: `TradeRecommendation.signal`/
`confidence_score` and each `HorizonView.signal`/`confidence_score` become *reflections of*
the Decision Engine's output, supplied to the LLM as a given rather than generated by it —
the LLM's role narrows to the commentary/rationale narrative fields only. This is an
adaptation of the existing personas' prompts and output contracts, not a new component.

**What this resolves**:
- Removes the single largest unvalidated scientific assumption named in the Final Review's
  §1 (whether the LLM personas have genuine predictive skill) — the question no longer
  needs answering, because the LLM no longer predicts anything.
- Eliminates the "correlated-origin votes counted as independent" risk for the 3
  Investor-horizon identities (Critical Review §3; Final Review §4/§5) — since they no
  longer vote, there is nothing left to double-count.
- Eliminates the LLM-engine-backtesting cost/feasibility problem (Critical Review §2) —
  there is no LLM prediction accuracy to backtest anymore, only explanation quality, a much
  cheaper and different kind of evaluation if ever pursued (not required for Phase 1).
- Strengthens the Decision Engine's accuracy-weighted voting itself — it now aggregates
  only components with an actual, establishable track record, a more scientifically
  defensible ensemble than one that included an unvalidated LLM opinion.

### Amendment 2 — Minimal Feature Store

**Change**: the Feature Store (previously designed with full production-grade scope in the
Methodology's §4) is scoped down for Phase 1 to exactly five components:

1. **Feature Versioning** — `feature_name@feature_version`; any computation-logic change
   bumps the version, never a silent redefinition.
2. **Point-in-Time correctness** — the mandatory `AsOfDataAccessor`; every feature
   retrievable only with data knowable as of a given timestamp, accounting for reporting lag
   (fundamentals/macro features timestamped by publication availability, not by the period
   they describe).
3. **Validation** — schema/range checks at write time (an out-of-range value, e.g. RSI
   outside [0,100], is quarantined immediately, not silently written).
4. **Redis Online Store** — the hot/live serving path, matching this codebase's existing
   `TTLCache`/`RedisCacheManager` pattern.
5. **PostgreSQL Offline Store** — the historical, versioned store used for training-dataset
   construction and backtesting, queried exclusively through the same `AsOfDataAccessor`.

**Explicitly excluded from Phase 1** (deferred to the Research Lab, Amendment 3, or a later
phase entirely): feature lineage tracking, feature-level drift/monitoring, formal
online/offline consistency-guarantee tooling beyond the basic two-store split, and any
feature-reuse-tracking system beyond the versioned name itself. This directly implements
the Final Review's Top-10 Highest-Value-Improvement #1 and Unnecessary-Complexity #2 — no
enterprise MLOps apparatus yet.

### Amendment 3 — Research Lab Module

**Change**: a new, explicitly internal-only subsystem, architecturally isolated from the
live prediction path. The live Engine Registry and Decision Engine never query it at
request time — data flows one way, from live operation *into* the Lab, never back out
synchronously.

```
ResearchLab
├── experiments                -- shadow-mode trials of new features/engines/models
│                                  (the shadow/promotion mechanism from Methodology §7
│                                  and the Critical Review now lives here concretely)
├── hypotheses                  -- pre-registered research questions, logged BEFORE
│                                  evaluation (the anti-cherry-picking safeguard from
│                                  Final Review §5; a durable home for the Final Review's
│                                  Top-10 Research Priorities list)
├── failed_models               -- an explicit graveyard: rejected shadow candidates and
│                                  formally deprecated engines/features, with the reason
│                                  for rejection/deprecation recorded (closes the
│                                  "research debt" risk named in Final Review §7 — nothing
│                                  lingers in limbo indefinitely)
├── feature_importance_history  -- see Amendment 4
├── benchmark_history            -- subsumes what the original architecture called
│                                  backtest_runs/benchmark_results; now conceptually
│                                  housed inside the Lab rather than bare standalone
│                                  production tables
└── weekly_research_reports      -- a structured periodic artifact summarizing the week's
                                    experiments/benchmarks/feature-importance trends
```

`weekly_research_reports` is a legitimate, safe use of an LLM under Amendment 1's new rule:
summarizing internal research data into a readable report for the research team is
narrative synthesis over already-computed evidence — exactly the role Amendment 1 assigns
to LLMs, just applied to internal research artifacts instead of user-facing
recommendations, and never itself feeding back into any live decision.

**Governing rule**: the Research Lab is where the Final Review's heavier, slower-cadence
recommendations live — the standing quarterly research-debt review (Final Review §7), the
on-demand (not scheduled) backtest/benchmark runs (Phase 1a/1b split, Critical Review), and
the obsolescence-vs-regime-dormant analysis (Methodology §7) all become concrete
*activities performed inside the Research Lab* by researchers reading its stored artifacts
— not automated production jobs polluting the live system's operational surface. This
directly closes the Final Review's "over-building now for a maturity level not yet reached"
risk (§7) by giving all of that apparatus an explicitly separate, lower-stakes home.

### Amendment 4 — Feature Importance History

**Change**: a concretely-specified table living inside the Research Lab (Amendment 3),
tracking how every feature's predictive value changes over time:

```
feature_importance_history
├── feature_name, feature_version
├── engine_name        -- importance is measured per consuming engine (a feature's
│                          SHAP/permutation-importance value, or its contribution-weight
│                          share within a rule-based engine, is engine-specific)
├── measured_at         -- timestamp of this measurement (e.g. weekly cadence, matching
│                          the weekly_research_reports cycle)
├── importance_score    -- normalized measure (permutation importance, SHAP magnitude, or
│                          contribution-weight share for rule-based engines)
├── importance_trend    -- rolling slope over recent measurements: increasing / flat /
│                          decreasing
└── regime_context      -- which market regime prevailed during the measurement window
```

**Purpose**: detect obsolete features and discover improving ones. `regime_context` is
what makes this actionable rather than merely descriptive — it lets the Research Lab
distinguish a feature that is **genuinely obsolete** (declining `importance_trend` across
*all* `regime_context` values over a sustained window — a `failed_models`/deprecation
candidate) from one that is merely **regime-dormant** (declining only within the
currently-prevailing regime, expected to matter again later) — the same distinction the
Methodology (§7) and Final Review both required, now given a concrete storage shape. A
shadow-mode feature (Amendment 3's `experiments`) whose `importance_trend` is *increasing*
over its evaluation window is symmetrically a promotion candidate — one table serves both
decay detection and improvement discovery.

### Updated end-to-end shape

```
Live request:
  Engine Registry (Tier A voting engines ONLY)
    → each engine reads Feature Store (Redis online / PostgreSQL offline, via AsOfDataAccessor)
    → produces EngineOutput{score, confidence, evidence, explanation, historical_accuracy}
  → Decision Engine (accuracy-weighted vote over Tier A engines only)
    → DecisionOutput{BUY|SELL|HOLD, confidence, contributing_engines, data_sufficiency, explanation}
  → Explanation Layer (Tier B, LLM: Trader Profile / Investor Profile)
    → consumes DecisionOutput + Tier A evidence
    → produces the user-facing Turkish narrative report
      (signal/confidence sourced FROM the Decision Engine, commentary generated BY the LLM)

Offline / research (never in the live path):
  Reconciliation job → engine_predictions / historical_accuracy (Tier A only — Tier B has
    no falsifiable prediction to reconcile)
  Research Lab ← historical engine outputs, decisions, feature values
    → experiments, hypotheses, failed_models, feature_importance_history,
      benchmark_history, weekly_research_reports
    → promotion/deprecation decisions applied back into the Engine Registry / Feature
      Store configuration by a human researcher — never automatically, never synchronously
```

### Explicit non-goals of this revision (per instruction)

No code. No implementation plan. No API design. These four amendments update the Phase 1
architecture design only, ahead of freezing it.
